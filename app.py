...
def process_input_data(df):
    ...
    data = data[(data['enmo'] != 0.0) | (data['diff_anglez'] != 0.0)]
...
if input_method == "Upload CSV file":
                ...
                with st.spinner("Processing data and making predictions..."):
                    ...
                    if predictions is not None:
                        ...
                        fig = make_subplots(rows=2, cols=1, 
                                        shared_xaxes=True, 
                                        subplot_titles=("Raw Movement Data", "Sleep Prediction"))
                        
                        # Raw data
                        fig.add_trace(
                            go.Scatter(x=processed_df['timestamp'], y=processed_df['anglez'], 
                                    name="Angle Z", line=dict(color="blue")),
                            row=1, col=1
                        )
                        fig.add_trace(
                            go.Scatter(x=processed_df['timestamp'], y=processed_df['enmo']*100, 
                                    name="Movement (scaled)", line=dict(color="green")),
                            row=1, col=1
                        )
                        
                        # Prediction
                        fig.add_trace(
                            go.Scatter(x=processed_df['timestamp'], y=processed_df['sleep_prediction'], 
                                    name="Sleep State", line=dict(color="red")),
                            row=2, col=1
                        )
                        
                        fig.update_layout(height=600, title_text="Sleep Analysis")
                        fig.update_yaxes(title_text="Movement Data", row=1, col=1)
                        fig.update_yaxes(title_text="Sleep State (1=Sleep, 0=Awake)", 
                                    tickvals=[0, 1], ticktext=["Awake", "Sleep"],
                                    row=2, col=1)
                        
                        st.plotly_chart(fig, use_container_width=True)
                        
                        # Show data table with predictions
                        with st.expander("View Processed Data with Predictions"):
                            st.dataframe(processed_df)
                        ...
                        st.download_button(
                            label="Download predictions as CSV",
                            data=csv,
                            file_name="manual_sleep_predictions.csv",
                            mime="text/csv",
                        )
    else:
        st.info("Add some data points to make predictions")

else:  # Use sample data
    st.subheader("Sample Data from final_series_df.csv")
    
    # Function to load sample data from final_series_df.csv
    @st.cache_data
    def load_sample_data():
        try:
            # Load the CSV file with only the needed columns
    ...
    if sample_df is not None:
        ...
        st.write("Sample data from final_series_df.csv:")
        st.dataframe(sample_df.head())
        
        # Function to get random samples
        def get_random_samples(df, n_samples=40):
            """Get n_samples random consecutive rows from the dataframe"""
            if len(df) <= n_samples:
                return df
            
            # Get a random starting index
            start_idx = np.random.randint(0, len(df) - n_samples)
            # Return the consecutive samples
            return df.iloc[start_idx:start_idx + n_samples].copy()
        
        # Button to randomize data
        if st.button("Get Random Data Sample"):
            # Store random data in session state
            st.session_state.random_sample = get_random_samples(sample_df)
            st.success("Random sample data selected!")
        
        # Check if random sample exists in session state
        if 'random_sample' in st.session_state:
            # Display the random sample
            st.subheader("Selected Random Sample")
            st.dataframe(st.session_state.random_sample)
            
            # Button to make prediction with this random sample
            if models_loaded and st.button("Make Prediction with Sample Data"):
                with st.spinner("Processing data and making predictions..."):
                    # Process data for prediction
                    input_df = st.session_state.random_sample.copy()
                    X_scaled, processed_df = process_input_data(input_df)
                    predictions = predict_sleep_state(models[selected_model], X_scaled)
                    ...
                    if predictions is not None:
                        ...
                        fig.add_trace(
                            go.Scatter(x=processed_df['timestamp'], y=processed_df['sleep_prediction'], 
                                      name="Sleep Prediction", line=dict(color="red")),
                            row=2, col=1
                        )
                        ...
                        st.download_button(
                            label="Download predictions as CSV",
                            data=csv,
                            file_name="sample_sleep_predictions.csv",
                            mime="text/csv",
                        )
        else:
            st.info("Click 'Get Random Data Sample' to select random data for prediction")
    else:
        st.error("Could not load final_series_df.csv. Make sure the file exists in the data directory.")

# Footer
st.markdown("---")
st.caption("Sleep State Prediction App | Created on May 3, 2025")
st.caption("This app uses XGBoost models trained on accelerometer data to predict sleep states.")